...
export class Gateway {
  ...
  constructor(options: GatewayOptions = {}) {
    ...
    // 3. Create draft manager (wired after WebSocket server is created)
    this.draftManager = new DraftManager({
      notifyWs: (event) => this.wsServer.broadcast(event),
  ...
  async start(): Promise<void> {
    ...
    console.log(`[gateway]   WebSocket: ws://localhost:${this.options.port ?? 8765}`);
  ...
  private async registerChannelAdapters(): Promise<void> {
    const enqueue = (msg: IncomingMessage) => {
